(() => {
  ...
  async function boot(){
    await loadScript('/data/decision-cards-v2.js?v=2.1.0');
    await loadScript('/app-v2.js?v=2.1.0');
    await loadScript('/features.js?v=2.1.0');
    if(document.readyState==='complete') window.dispatchEvent(new Event('load'));
  }

  boot().catch(error=>{
    console.error('Forked Kingdoms 2.1 boot failed:',error);
